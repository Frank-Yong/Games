...
if(!empty($_REQUEST['reassign'])) {
	...
	if($_SESSION['USERID'] == $_REQUEST['uid']) {
		$sql = "SELECT id FROM reassign WHERE userid=".$_SESSION['USERID'];
		...
		if($nr<=1) {
			...
			$sql = "INSERT INTO reassign (userid, playerid, post, data)
					VALUES(".$_SESSION['USERID'].",".$_REQUEST['pid'].", ".$_REQUEST['postnou'].",'$disponibil')";
			mysqli_query($GLOBALS['con'], $sql);
			$sql = "UPDATE lineup SET post=0 WHERE playerid=".$_REQUEST['pid'];
			mysqli_query($GLOBALS['con'], $sql);
			...
			$_SESSION['_MESSAGE'] = 'Only 2 players can be reassign in the same time!';		
		}
	}
}
...
if(!empty($_REQUEST['SendSigla'])) {
	...
	if ($_FILES['file_up']['size']>150000){
		$_SESSION['_MESSAGE'] = 'Logo too big. It must be under 150KB!';		
		$file_upload="false";
	}
	...
	if (!($_FILES['file_up']['type'] =="image/jpg" || $_FILES['file_up']['type'] =="image/jpeg" || $_FILES['file_up']['type'] =="image/gif")){
		$_SESSION['_MESSAGE'] = 'File type incorrect!';
		$file_upload="false";
	}
	...
	if($file_upload=="true"){
		...
		if(move_uploaded_file ($_FILES['file_up']['tmp_name'], $add)){
			$sql = "UPDATE user SET pic='$add' WHERE id=".$_SESSION['USERID'];
			mysqli_query($GLOBALS['con'], $sql);
			...
			exit;
					...
					$_SESSION['_MESSAGE'] = 'There was a problem and the action cannot be processed!';
		}
	}else{
		echo $msg;
	}  
...
if(!empty($_REQUEST['AlegeAntrenament'])) {
	...
	while(list($pid) = mysqli_fetch_row($res)) {
		$update = "UPDATE trainerplayer SET post = ". $_REQUEST['post_'.$pid] . ", trainerid=".$_REQUEST['antrenor_'.$pid].", ttype=".$_REQUEST['tip_'.$pid]." WHERE playerId=$pid";
		//echo "$update<br/>";
...
if(isset($_REQUEST['option'])) {
	...
	if($_REQUEST['option'] == "messages") {
		if(isset($_REQUEST['accept'])) {
			...
			$sql = "UPDATE gameinvitation SET accepted=1 WHERE id=".$_REQUEST['accept'];
			mysqli_query($GLOBALS['con'], $sql);
			
			$sql = "SELECT userId_1, userId_2, datameci FROM gameinvitation WHERE id=".$_REQUEST['accept'];
			$res = mysqli_query($GLOBALS['con'], $sql);
			...
			$sql = "INSERT INTO etapa(iduser1, iduser2, data, idstadium)
					SELECT userId_1, userId_2, datameci, stadium FROM gameinvitation
		...
		if(isset($_REQUEST['deny'])) {
			//s-a primit accept pt un amical
			$sql = "UPDATE gameinvitation SET accepted=0 WHERE id=".$_REQUEST['deny'];
			mysqli_query($GLOBALS['con'], $sql);
		}
	}
...
if(!empty($_REQUEST['SetEveniment'])) {
	...
	$num=0;
	$sql = "SELECT id FROM gameinvitation WHERE accepted=1 AND datameci='$data' AND (userId_1=".$_SESSION['USERID'] . " OR userId_2=". $_SESSION['USERID']. ")";
	$res = mysqli_query($GLOBALS['con'], $sql);
...
if(isset($_REQUEST['SetAmical'])) {
	//insert in 'gameinvitation' the game request, with field accepted=-1 (default)
	//insert into the 'messages' table the invitation
	//on 'accept', set accepted=1
	//on 'deny', set accepted = 0 
	//in 'messgaes', in case invitation is sent, set on gameID ("meciID" in table) the id of the game
	//if the message is not about game invitation, field gameID remains empty
	...
	if($data_curenta>$data) {
		$_SESSION['_MESSAGE'] = 'Invitation has not been sent, date is unavailable!';
	} else {
		...
		if($_REQUEST['stadion'] == $stadionadv) {
			//away
			$sql = "INSERT INTO gameinvitation (UserId_1, UserId_2, gametype, gamedate, age, stadium, accepted)
		...
		$subject = "Game invitation against $nume_adversar";
		$body = "<b>$nume_adversar</b> footbal team asks you to play a friendly game, on $data, location: $nume_stadion. To accept it, please check below!";

		$sql = "INSERT INTO messages (fromID, toID, subject, body, data, meciID)
				VALUES (".$_SESSION['USERID'].", ".$_REQUEST['club_id']. ", '$subject', '$body', '$data_curenta', $meci_id)";
		mysqli_query($GLOBALS['con'], $sql);
		
		$mes = "Hey there!\r\n\r\nYou have received a game invitation! Check if you can have free spot in calendar for it!\r\nhttp://locahost";
		$mes = wordwrap($mes, 70, "\r\n");
		if($estebot) {
		} else {
			mail($emailadv, 'Game Invitation!', $mes);
			mail('fcbrasov@yahoo.com', 'Game invitation myfm.com!', $mes);
		}
...
*/
	...
	while(list($pid) = mysqli_fetch_row($res)) {
		...
		*/
		...
		$_SESSION['_MESSAGE'] = "Line-up is up-to-date!";
	}
...
?>
			...
			<div class="container-3 right">
				<h3>Contact</h3>
				<div class="containet-3-text">
					Please contact us at contact@myfm.com .
				</div>
			</div>
			<div class="clear"></div>
		</div>
