<!--
The ranking is displayed according to the league it belongs to and the season.
In the competition table, a competition is created for each season, and the id goes further in the ranking

After each stage, at 13:50, the script is run that introduces the results in the ranking, in the sense that it covers all the leagues and depending on the scores, inserts the necessary data in the ranking table. 
-->
...
<table class="tf2"  width="100%">
					<tr>
						...
						-->
						<th><font color="<?php echo $gk; ?>">W</font></th>
						<th><font color="<?php echo $gk; ?>">D</font></th>
						<th><font color="<?php echo $gk; ?>">L</font></th>
						<th><font color="<?php echo $gk; ?>">Goals</font></th>
						
						<th><font color="<?php echo $gk; ?>">Points</font></th>
					</tr>
...
//egaluri - draws
//infringeri - defeats
//gm - goals scored, gp - goals received, puncte = points
$sql = "SELECT b.id, b.TeamName, c.name, SUM(a.victorii), SUM(a.egaluri), SUM(a.infringeri), SUM(a.gm), SUM(a.gp), SUM(a.puncte)
		FROM clasament a
		LEFT JOIN user b
		ON b.id=a.userId
...
$res = mysqli_query($GLOBALS['con'], $sql);
$index=1;
while(list($echipaid, $echipa, $liga, $v,$e, $inf, $gm, $gp, $puncte) = mysqli_fetch_row($res)) {
?>
					<?php
					if($index<2) {
					?>
...
<tr>
	<td colspan="7">
		...
		<table class="tf2" width="100%">
			...
			<tr>
				<th colspan="2">
				<input type="Submit" name="AfisareClasament" value="Display" class="button-2">
				</th>
			</tr>
		</table>
		</form>
