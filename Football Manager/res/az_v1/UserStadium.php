...
class user {
	...
	public function LoginID($userid) {
		...
		//echo "$sql<br/>";
		$res = mysql_query($sql);
		list($user_id, $team_name, $username, $rating, $funds, $lid, $StadiumID, $lastactive, $leaguename, $pic, $mor) = mysql_fetch_row($res);
		...
		if ($user_id > 0) {
			...
			//echo "$sql<br/>";
		    $res = mysql_query($sql);
			list($online) = mysql_fetch_row($res);
			$this->Online=$online;
			mysql_free_result($res);
		} else {
	...
	public function Login($username, $password) {
		...
		//echo "$sql<br/>";
		$res = mysql_query($sql);
		list($user_id, $team_name, $username, $rating, $funds, $StadiumID, $email, $lastactive, $activated, $leaguename, $pic, $mor) = mysql_fetch_row($res);
		mysql_free_result($res);
		...
		if ($user_id > 0) {
			...
			$_SESSION['_MESSAGE'] = "Updating...!!!";//"Utilizator sau parola gresite!";
			$_SESSION['badlogin'] = 1;
		}
	}
	...
	echo "Utilizator ID: " . $this->UserID.'';
	echo "<div class=\"hr-replace\"></div>";
	$s = new Stadium($this->StadiumID);
	$numestad = $s->ReturnStadiumName();
	echo "Stadion: " . $numestad;
	echo "<div class=\"hr-replace\"></div>";
	$liga = $this->LeagueID == 0? 'nealocat': $this->LeagueName;
	echo "Liga: " . $liga."<br/>";
	$activ = $this->Online == 1 ? 'Activ':'Inactiv';
	//echo "Online: $activ</br/>"; 
	echo "Activ ultima data: " . $this->LastActive;
	echo "<div class=\"hr-replace\"></div>";
	if($_SESSION['USERID'] == $this->UserID)
		echo "<b>Bani: " . number_format($this->Funds) . " &euro;</b>";
	...
	public function ComputeTeamValues($pgroup=1) {
		...
		while(list($p_id, $p_position, $heisusedlike, $tactics, $midfield, $atacks, $passes) = mysql_fetch_row($res)) {
...
class Stadium {
	...
	public function Stadium($id=0, $Name, $Sector1=50, $Sector2=400, $Sector3=50, $Sector4=200, $Sector5=50, $Sector6=400, $Sector7=50, $Sector8=200, $Sector1c=1, $Sector2c=2, $Sector3c=1, $Sector4c=2, $Sector5c=1, $Sector6c=2, $Sector7c=1, $Sector8c=2) {
		...
		if(func_num_args()==1) {
			//se trimite doar $ID
				...
				$co1, $co2, $co3, $co4, $co5, $co6, $co7, $co8, $da1, $da2, $da3, $da4, $da5, $da6, $da7, $da8) = mysql_fetch_row($res);
				$this->Name = $name;
...
echo "plllllll";
	...
	function ViewStadium() {
		$constructie = "<font color=\"red\">Sector in constructie</font><br/>";
		echo "<div>";
		echo "Nume stadion: ".$this->ReturnStadiumName().'<br/>';
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 1 (S1): ".$this->Sector1.' locuri<br/>';
		if($this->Construction1>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 2 (S2): ".$this->Sector2.' locuri<br/>';
		if($this->Construction2>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 3 (S3): ".$this->Sector3.' locuri<br/>';
		if($this->Construction3>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 4 (S4): ".$this->Sector4.' locuri<br/>';
		if($this->Construction4>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 5 (S5): ".$this->Sector5.' locuri<br/>';
		if($this->Construction5>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 6 (S6): ".$this->Sector6.' locuri<br/>';
		if($this->Construction6>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 7 (S7): ".$this->Sector7.' locuri<br/>';
		if($this->Construction7>0) echo $constructie;
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 8 (S8): ".$this->Sector8.' locuri<br/>';
	...
	function BuildStadium() {
		echo "<div class=\"hr-replace\"></div>";
		echo "<h2>Mareste capacitatea!</h2><br/><font color=\"red\">(Priceul de constructie al unui scaun este 500 &euro;. Marirea capacitatii stadionului se face doar daca exista fonduri suficiente pentru constructie. In perioada de constructie, acel sector nu va fi Available pentru spectatori. Cu cat sunt mai multe sectoare in constructie, cu atat durata de finalizare este mai mare!)</font>";
		...
		if($this->Construction1>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction1. " de locuri se va termina in ". $this->Data1."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s1_build\" class=\"input-1\" value=\"0\" $disabled id=\"s1\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 2 (S2): ".$this->Sector2.' locuri<br/>';
		$disabled="";
		if($this->Construction2>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction2. " de locuri se va termina in ". $this->Data2."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s2_build\" class=\"input-1\" value=\"0\" $disabled id=\"s2\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 3 (S3): ".$this->Sector3.' locuri<br/>';
		$disabled="";
		if($this->Construction3>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction3. " de locuri se va termina in ". $this->Data3."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s3_build\" class=\"input-1\" value=\"0\" $disabled id=\"s3\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 4 (S4): ".$this->Sector4.' locuri<br/>';
		$disabled="";
		if($this->Construction4>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction4. " de locuri se va termina in ". $this->Data4."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s4_build\" class=\"input-1\" value=\"0\" $disabled id=\"s4\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 5 (S5): ".$this->Sector5.' locuri<br/>';
		$disabled="";
		if($this->Construction5>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction5. " de locuri se va termina in ". $this->Data5."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s5_build\" class=\"input-1\" value=\"0\" $disabled id=\"s5\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 6 (S6): ".$this->Sector6.' locuri<br/>';
		$disabled="";
		if($this->Construction6>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction6. " de locuri se va termina in ". $this->Data6."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s6_build\" class=\"input-1\" value=\"0\" $disabled id=\"s6\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 7 (S7): ".$this->Sector7.' locuri<br/>';
		$disabled="";
		if($this->Construction7>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction7. " de locuri se va termina in ". $this->Data7."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s7_build\" class=\"input-1\" value=\"0\" $disabled id=\"s7\"> locuri";
		echo "<div class=\"hr-replace\"></div>";
		echo "Sector 8 (S8): ".$this->Sector8.' locuri<br/>';
		$disabled="";
		if($this->Construction8>0) {
			$disabled="disabled";
			echo "Constructia celor ".$this->Construction8. " de locuri se va termina in ". $this->Data8."!<br/>";
		}
		echo "Mareste capacitatea cu: <input type=\"text\" name=\"s8_build\" class=\"input-1\" value=\"0\" $disabled id=\"s8\"> locuri";
		echo "<br/><br/>";
		echo "<input type=\"Submit\" name=\"MaresteCapacitate\" value=\"Mareste capacitate\" class=\"button-2\">";
		echo "</form>";
	}
