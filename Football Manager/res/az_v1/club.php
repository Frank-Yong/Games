...
if(!empty($_REQUEST['AlegeTricou'])) {
	//$_REQUEST['numar']::$_REQUEST['juc']::$_SESSION['USERID']
	//verific mai intii daca umarul exista in acea echipa
...
//	echo "Numar de numere : ".$nrnumar.'<br/>';
	if($nrnumar>0 && $numar<>0) {	
		...
		$sql = "UPDATE userplayer SET numar=$numar WHERE playerid=$pid AND userid=".$_SESSION['USERID'];
//		echo "$sql<br/>";
		mysql_query($sql);
	}
